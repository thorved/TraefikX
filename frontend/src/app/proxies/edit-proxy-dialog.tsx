...
export function EditProxyDialog({
  ...
  useEffect(() => {
    if (proxy) {
      setDomainNames(proxy.domain_names);
      setDomainErrors(new Array(proxy.domain_names.length).fill(null));
      setForwardScheme(proxy.forward_scheme as "http" | "https");
      setForwardHost(proxy.forward_host);
      setForwardPort(proxy.forward_port.toString());
      setSsl(proxy.ssl);
      setAccess(proxy.access as "public" | "private");
    }
  }, [proxy]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <DialogFooter>
          ...
          >
            {updateProxy.isPending ? "Updating..." : "Update"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
